...
const formSubmit = e => {
    ...
    e.preventDefault();
    if(!uniqueName.value.trim()) confirmationShow.innerText = 'Enter your_text to appear in the url';
    else {
        ...
        .then(response => {
            ...
            if(!response.ok){
                ...
                e.target.reset()
                confirmationShow.innerHTML = `Urekaa!!! The link can now be visited via <a target="_blank" href="https://${response.newUrl}"> ${response.newUrl} </a>`
        ...
        .catch(err => {
            ...
            status.innerHTML = '<button class="btn-shorten">Shorten Link</button>';
            confirmationShow.innerText = "Oops, connection lost!";
        })
    };
}
